"""Errors and the failure taxonomy for the isolated render boundary.

``DockerError`` is the render worker's original error. The failure taxonomy
alongside it is the repair loop's shared vocabulary: every pipeline failure
classifies into a typed record mapping the preflight gate names (#80) and
critic check names (#82) onto repair categories -- a mapping, never string
re-parsing.
"""
...
def classify_provider_error(error: Exception) -> FailureClassification:
    """Map a provider/LLM failure onto the provider category."""
    return FailureClassification(
        type="provider",
        category="provider_error",
        suggestion=f"The provider call failed: {error}. Retry once conditions recover.",
        evidence=str(error),
    )
