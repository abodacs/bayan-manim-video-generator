"""Test doubles for offline development and tests -- explicitly not product code.

``FakeProvider`` is the deterministic stand-in for every LLM seam (lesson
plans, scene code, repairs). The only product user is the legacy offline
path, ``bayan run --provider fake``, which composes it in
:mod:`bayan.planner.service`; the generate pipeline never touches it. Its
repair heuristic derives the allowed imports from the preflight allowlist
(:data:`bayan.pipeline.preflight.ALLOWED_MODULES`) so pipeline policy lives
in exactly one place: when a gate tightens, the fake's repairer tightens
...
from bayan.pipeline.preflight import ALLOWED_MODULES
...
def imports_allowed_module(line: str) -> bool:
    """Whether one source line imports a module on the preflight allowlist.

    The repair heuristic keeps exactly the import lines the gates accept:
    the module name is parsed out of the line and checked against
    ``ALLOWED_MODULES`` (never prefix-matched, so ``import mathx`` cannot
    ride on ``import math``).
    """
    parts = line.split()
    return len(parts) >= 2 and parts[0] in {"import", "from"} and parts[1] in ALLOWED_MODULES
    ...
    def repair_scene_code(
        ...
        if classification.category == "disallowed_import":
            lines = [
                ...
                if not (line.startswith(("import ", "from ")) and not imports_allowed_module(line))
            ]
            code = "\n".join(lines) + "\n"
